...
app.controller('KYCCtrl', ['$scope', 'HTTP', 'Upload', ($scope, HTTP, Upload) => {
  ...
  function updateKYCDetails (kyc) {
    ...
    HTTP.post(`/users/${$scope.activeUserProfile.id}/kycdata`, kyc)
      .fork(fail, d => {
  ...
  function uploadKYCDocument (ukd) {
    ...
    Upload.upload({
      url: `/users/${$scope.activeUserProfile.id}/kyc-documents`,
      data: ukd
    }).then(d => {
  ...
  function osvRequest (doc) {
    HTTP.post(`/users/${$scope.activeUserProfile.id}/onsite-verification-requests`, {notes: ''})
      .fork(e => {

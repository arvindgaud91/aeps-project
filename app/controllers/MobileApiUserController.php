...
class MobileApiUserController extends BaseController
{
	...
	public function postLoginAgentV2 ()
	{
		if (! Input::has('mobile_auth_code')) return Response::json('Mobile Auth Code missing', 422);
		$credentials = ['phone_no' => Input::get('uid', ''), 'password' => Input::get('password', '')];
		$user = Auth::validate($credentials);
	  if (! $user)
	  {
	  ...
	  if ($user->mobile_auth_code != NULL && $user->mobile_auth_code != Input::get('mobile_auth_code')) {
	  	return Response::json('User is currently logged in from another device.', 409);
	  }
	  ...
	  $user->mobile_auth_code = Input::get('mobile_auth_code');
	  $user->save();
  ...
  //   Event::fire('user.password.reset-request', [$pwdToken]);
    ...
    if ($pwdToken && ((new DateTime)->getTimestamp() - $pwdToken->created_at->getTimestamp()) < 50 * 60) {
		...
		SMS::send($user->phone_no, 'Your OTP is '.$pwdToken->otp.'. Digital India Payments.');
		return Response::json('', 200);
	}

	public function postRequestResetSession ()
	{
		$user = User::where('phone_no', Input::get('phone_no', ''))
			->where('type', 4)
			->first();
		...
		$otpObj->save();
		SMS::send($user->phone_no, 'Your OTP is '.$otp.'. Digital India Payments.');
		return Response::json('', 200);
	}

	public function postResetSession ()
	{
		if (! Input::has('phone_no') || ! Input::has('otp') || ! Input::has('password') || ! Input::has('mobile_auth_code')) {
			return Response::json('Missing info', 400);
		}
		$user = User::where('phone_no', Input::get('phone_no'))->first();
		if (! $user) return Response::json('Phone number not registered with us', 400);
		...
		$loggedInUser = Auth::validate(['phone_no' => Input::get('phone_no', ''), 'password' => Input::get('password', '')]);
	    ...
	    if ($loggedInUser->phone_verified == 0) {
	      return Response::json('Please verify the mobile number', 401);
	    }
	    ...
	    $vendor = Vendor::where('user_id', $loggedInUser->id)->first();
		$rblResponse = $this->loginRBL($vendor);
		if (! $rblResponse || $rblResponse->responseCode == '01')
		{
			// \Log::info(json_decode($rblResponse));
		...
		$loggedInUser->mobile_auth_code = Input::get('mobile_auth_code');
		$loggedInUser->save();
		...
		return Response::json('success', 200)->header('auth', Auth::generateLoginToken($loggedInUser));
	}

	public function postResendResetSessionOTP ()
	{
		if (! Input::has('phone_no')) return Response::json('Data is missing', 400);
		$user = User::where('phone_no', Input::get('phone_no'))->first();
		if (! $user) return Response::json('Phone number not registered with us', 400);
		...
		$sessionToken = SessionResetOTP::where('user_id', $user->id)
			->orderBy('created_at', 'DESC')
			->first();
		if ($sessionToken && ((new DateTime)->getTimestamp() - $sessionToken->created_at->getTimestamp()) < 50 * 60) {
		} else {
			$otp = mt_rand(1000, 9999);
			$sessionToken = PasswordResetOTP::create(['user_id' => $user->id, 'otp' => $otp.'', 'ip' => Request::getClientIp()]);
		}
		SMS::send($user->phone_no, 'Your OTP is '.$sessionToken->otp.'. Digital India Payments.');
		return Response::json('', 200);
	}
	...
	public function getProfile ()
	{
		...
		return Response::json([
			...
			"udc" => $user->vendorDetails->device_id,
			"device_id"=> $user->vendorDetails->fingerprint_device_id,
			"device_service"=>$user->vendorDetails->device_service
		], 200);
	}
	...
	public function getLogout ()
	{
		...
		$expToken->save();
		User::where('id', Auth::user()->id)->update(['mobile_auth_code' => NULL]);
		Auth::logout();
		return Response::json('', 200);
	}
